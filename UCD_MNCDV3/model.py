...
import os
from utils import dataset, dataset_MNCDV3
...
class Change_Detection_Framework(nn.Module, PyTorchModelHubMixin):
    ...
    def training_CD(self):
        ...
        best_f1, best_f1_epoch, best_c_f1, best_c_f1_epoch =0,0,0,0
        iterations_per_epoch=len(self.CD_dataloader_train)

        if self.accelerator.is_local_main_process:
            self.summarywriter=SummaryWriter(f'runs/{self.configs["model_name"]}_{self.configs["dataset_name"]}')
        for epoch in range(self.configs["train"]["epochs"]):
            ...
            for i, batch in enumerate(tqdm(self.CD_dataloader_train, disable=not self.accelerator.is_local_main_process, miniters=20)):
                ...
                # print("pre_tensor", pre_tensor.shape, "post_tensor", post_tensor.shape)
                if label_tensor.shape[0]=="1":
                    label_tensor.squeeze().unsqueeze()
                else:
                    label_tensor.squeeze()
                
                # print(self.device, pre_tensor.device, post_tensor.device)
                ...
                if self.configs["train"]["loss"]=="integrated": #MineNetCD/MaskCD
                    ...
                    if self.configs["train"]["loss"]=="BCE" or self.configs["train"]["loss"]=="Dice":
                        ...
                        # print(total_loss)
                if self.accelerator.is_local_main_process:
                    self.summarywriter.add_scalar('loss', total_loss.item(), iterations_per_epoch*epoch+i)
            ...
            if self.accelerator.is_local_main_process:
                self.summarywriter.add_scalar('avg_loss_per_epoch', loss_avg, epoch)
            if self.lr_scheduler is not None:
                self.lr_scheduler.step()
            # print(f'Epoch {epoch} training completed, the average loss is {loss_avg}. learning rate is {self.lr_scheduler.get_lr()}')
            self.accelerator.print(f'Epoch {epoch} training completed, the average loss is {loss_avg}')
            ...
            if (epoch+1)%self.configs["train"]["save_intervals"]==0:
                ...
                if self.configs["eval"]:
                    f1, c_f1=self.evaluating_CD()
                    if self.accelerator.is_local_main_process:
                        self.summarywriter.add_scalar('evaluated_f1', f1, epoch)
                    if f1>best_f1:
        ...
        if self.accelerator.is_local_main_process:
            print(f"Training Finished, BestF1 is {best_f1} achieved in Epoch {best_f1_epoch}, BestCF1 is {best_c_f1} achieved in Epoch {best_c_f1_epoch},")
    ...
    def evaluating_CD(self):
        ...
        for _, batch in enumerate(tqdm(self.CD_dataloader_eval, disable=not self.accelerator.is_local_main_process, miniters=20)):
            pre_tensor, post_tensor, label_tensor, fname = batch["pre"], batch["post"], batch["gt"], batch["fname"]
            pre_tensor = pre_tensor.to(self.device)
            post_tensor = post_tensor.to(self.device)
            label_tensor = label_tensor.to(self.device)
            ...
            for i in range(prediction.shape[0]):
                ...
                if self.configs["eval"]["evaluator"]=="torchmetrics":
                    self.f1.update(target=label_tensor[i].flatten(),preds=prediction[i].flatten())
                else:
        ...
        final_metric=final_metric if self.configs["eval"]["evaluator"]=="torchmetrics" else final_metric[0]
        self.accelerator.print(f'evaluated f1 is {f1}, mf1 is {final_metric}, cf1 is {f1[1]}')
        return final_metric, f1[1]
    ...
    def testing_CD(self):
        ...
        for _, data in enumerate(tqdm(self.CD_dataloader_test, disable=not self.accelerator.is_local_main_process, miniters=20)):
            ...
            if self.configs["num_classes"]>2:
                self.f1.update(target=label_tensor, preds=prediction)
            else:
        ...
        if self.configs["num_classes"]>2:
            ...
            self.accelerator.print(f'Tested f1 is {final_f1}, mean is {torch.mean(final_f1, dim=0)}')
        else:
    ...
    def _get_metrics(self):
        if self.configs["eval"]["evaluator"]=="torchmetrics":
            self.f1=torchmetrics.F1Score(task="multiclass", num_classes=self.configs["num_classes"], average=None)
            self.f1=self.accelerator.prepare(self.f1)
        else:
    ...
    def _get_CD_dataloader(self):
        ...
        print("dataset type is:", self.configs["data_type"])
        ...
        elif self.configs["data_type"]=="MNCD":
            self.__get_CD_dataloader_MNCD()
        elif self.configs["data_type"]=="MNCDV3":
            self.__get_CD_dataloader_MNCDV3()

    def __get_CD_dataloader_MNCD(self):
        if self.mode=="train":
            CD_dataset_train=dataset.Bitemporal_Change_Detection_Dataset(self.configs["dataset_path"], year_range=(2015,2024), mode='train')
            ...
            if self.configs["eval"]:
                CD_dataset_eval=dataset.Bitemporal_Change_Detection_Dataset(self.configs["dataset_path"], year_range=(2015,2024), mode='val')
            ...
            self.accelerator.print("Number of training samples:", len(CD_dataset_train), "Number of evaluating samples:", len(CD_dataset_eval))
        elif self.mode=="test":
            CD_dataset_test=dataset.Bitemporal_Change_Detection_Dataset(self.configs["dataset_path"], year_range=(2015,2024), mode='test')
            ...
            self.accelerator.print("Number of testing samples:", len(CD_dataset_test))

    def __get_CD_dataloader_MNCDV3(self):
        if self.mode=="train":
            CD_dataset_train=dataset_MNCDV3.MNCDV3_Dataset(root_path=self.configs["dataset_path"], normalization=True, mode='train')
            CD_dataloader_train=data.DataLoader(dataset=CD_dataset_train,batch_size=self.configs["train"]["batch_size"],shuffle=True,num_workers=0,pin_memory=False,collate_fn=None)
            self.CD_dataloader_train=self.accelerator.prepare_data_loader(CD_dataloader_train)
            if self.configs["eval"]:
                CD_dataset_eval=dataset_MNCDV3.MNCDV3_Dataset(root_path=self.configs["dataset_path"], normalization=True, mode='val')
                CD_dataloader_eval=data.DataLoader(dataset=CD_dataset_eval,batch_size=self.configs["eval"]["batch_size"],shuffle=False,num_workers=0,pin_memory=False,collate_fn=None)
                self.CD_dataloader_eval=self.accelerator.prepare(CD_dataloader_eval)
            self.accelerator.print("Number of training samples:", len(CD_dataset_train), "Number of evaluating samples:", len(CD_dataset_eval))
        elif self.mode=="test":
            CD_dataset_test=dataset_MNCDV3.MNCDV3_Dataset(root_path=self.configs["dataset_path"], normalization=True, mode='test')
            CD_dataloader_test=data.DataLoader(dataset=CD_dataset_test,batch_size=self.configs["test"]["batch_size"],shuffle=False,num_workers=0,pin_memory=False,collate_fn=None)
            self.CD_dataloader_test=self.accelerator.prepare_data_loader(CD_dataloader_test)
            self.accelerator.print("Number of testing samples:", len(CD_dataset_test))
