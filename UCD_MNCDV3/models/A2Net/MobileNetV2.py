...
class MobileNetV2(nn.Module):
    def __init__(self, pretrained=None, num_classes=1000, width_mult=1.0, in_channel=3):
...
def mobilenet_v2(pretrained=True, progress=True, in_channel=3, **kwargs):
    model = MobileNetV2(in_channel=in_channel, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['mobilenet_v2'],
                                              progress=progress)
        print("loading imagenet pretrained mobilenetv2")
        if in_channel!=3:
            state_dict.pop("features.0.0.weight")
