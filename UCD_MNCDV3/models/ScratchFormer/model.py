...
from encoder import EncoderBlock, CEFF, LayerNorm
...
class ScratchFormer(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        return cp
        return {"main_predictions": cp[-1], "aux_predictions": cp[:-1]}
    
if __name__=="__main__":
    # Only using 128/256/512/...
    model=ScratchFormer(3,2)
    device=torch.device("cuda")
    model=model.to(device)
    a=torch.rand(8,3,128,128).to(device)
    b=torch.rand(8,3,128,128).to(device)
    c=model(a,b)
    for cc in c:
        print(cc.shape)