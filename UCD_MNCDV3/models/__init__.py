...
def find_model_using_name(configs):
    ...
    if model_name== "A2Net":
        ...
        print("BIT input_channels:",configs["input_nc"])
        ...
        model=module.DMINet(num_classes=configs["num_classes"], in_chans=configs["input_nc"])
        ...
        model=module.ICIFNet(num_classes=configs["num_classes"], in_chans=configs["input_nc"])
        ...
        model=module.ChangeClassifier(in_channels=configs["input_nc"])
        ...
        model = module.AFCF3D(configs["input_nc"])
    elif model_name== "CGNet":
        module=importlib.import_module('.cgnet',package='.models')
        model = module.CGNet(in_chans=configs["input_nc"])
    elif model_name== "HCGMNet":
        module=importlib.import_module('.cgnet',package='.models')
        model = module.HCGMNet(in_chans=configs["input_nc"])
        ...
        model = module.SwinSUNet(in_chans=configs["input_nc"], num_classes=configs["num_classes"])
    elif model_name== "MSPSNet":
        module=importlib.import_module('.MSPSNet',package='.models')
        model = module.FEBlock1(in_ch=configs["input_nc"], ou_ch=configs["num_classes"])
    elif model_name== "DTCDSCN":
        module=importlib.import_module('.DTCDSCN',package='.models')
        model = module.CDNet34(in_channels=configs["input_nc"],num_classes=configs["num_classes"])
    elif model_name== "MineNetCD":
        module=importlib.import_module('.MineNetCD.minenetcd',package='.models')
        model=module.get_model_minenetcd(backbone_type=configs["model"]["backbone_type"] if "model" in configs and "backbone_type" in configs["model"] else "Swin_Diff_T",channel_mixing=True,num_classes=configs["num_classes"])
    else:
