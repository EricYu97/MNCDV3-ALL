...
class ViT_Baseline(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=9, num_classes=1000,
        ...
        self.neck=MultiLevelNeck(in_channels=[768, 768, 768, 768],out_channels=768,scales=[4, 2, 1, 0.5]).cuda()
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in [2, 5, 8, 11]:
                B, _, C = x.shape
                out = x.reshape(B, 14, 14,
                                  C).permute(0, 3, 1, 2).contiguous()
                features.append(out)

        # For multiscale UperNet
...
if __name__=="__main__":
    model=TIMMVisionTransformer(img_size=224, patch_size=16, in_chans=9, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., layer_scale=True,
                 embed_layer=PatchEmbed, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 act_layer=nn.GELU, window_attn=False, window_size=14, pretrained=None,
                 with_cp=False)
    model=model.cuda()
    neck=MultiLevelNeck(in_channels=[768, 768, 768, 768],out_channels=768,scales=[4, 2, 1, 0.5]).cuda()
    input=torch.rand(8,9,224,224).cuda()
