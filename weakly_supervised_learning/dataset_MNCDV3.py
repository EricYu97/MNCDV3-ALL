...
class MNCDV3_Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_root, normalization=None, mode='train', filter_empty=False, cache_file='mask_stats.npy', temporal="T1"):
        self.root_path = dataset_root
        self.normalization = normalization
        self.mode=mode
        self.temporal=temporal  # 'T1', 'T2' or 'bitemporal'
    ...
    def get_domain_data_list(self, root_path):
        ...
        for domain in self.domains:
            ...
            # For segmentation
            if self.temporal=="T1":
                patches = [f"{s}#pre" for s in patches]
            elif self.temporal=="T2":
                patches = [f"{s}#post" for s in patches]
            elif self.temporal=="bitemporal":
                patches = [f"{s}#pre" for s in patches] + [f"{s}#post" for s in patches]
            else:
                raise ValueError("temporal must be 'T1', 'T2' or 'bitemporal'")
    ...
    def get_sample(self, sample):
        domain, sample_id, time = sample.split('#')
        sample_path = os.path.join(self.root_path, domain)
    ...
    def __getitem__(self, idx):
        ...
        image, label = self.get_sample(sample)

        return {"images": image, "labels": label}
        # return {"pre":pre_image, "post": post_image, "gt": change_label, "fname": []}

class MNCDV3_WSSCD_Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_root, normalization=None, mode='train', filter_empty=False, cache_file='mask_stats.npy'):
        self.root_path = dataset_root
        self.normalization = normalization
        self.mode=mode
        self.filter_empty = filter_empty if mode == 'train' else False  # val/test 不过滤
    ...
    def get_sample(self, sample):
        ...
        # Concatenate pre/post images along channel dim for model input
        images={"pre": pre_image, "post": post_image}
        labels={"pre": pre_label, "post": post_label, "change_mask": change_mask, "image_label": image_label}
    ...
    def __getitem__(self, idx):
        ...
        # return {"pre":pre_image, "post": post_image, "gt": change_label, "fname": []}
def main():
    root_path='/Users/weikangyu/Documents/MNCDV3-ALL/MNCDV3_Bitemporal_Cropped_Size224_Step112'
    dataset = MNCDV3_WSSCD_Dataset(root_path, normalization=True)
    ...
    for i, batch in enumerate(dataloader):
        print(batch["images"])
        pre_image = batch["images"]["pre"]
        post_image = batch["images"]["post"]
        pre_label = batch["labels"]["pre"]
        post_label = batch["labels"]["post"]
        change_label = batch["labels"]["change_mask"]
        image_label = batch["labels"]["image_label"]

        print("pre_image:", pre_image.shape, "post_image:", post_image.shape)
        print("pre_label:", pre_label.shape, "post_label:", post_label.shape, "change_label:", change_label.shape, "image_label:", image_label)
        # print(f'Batch {i}:')
