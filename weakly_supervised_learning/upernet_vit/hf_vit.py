...
class ViTModel(ViTPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        reshaped_features=[]
        for feature in encoder_outputs.last_hidden_state:
            if feature is not None:
                B, N, C = feature.shape
                x = feature[:, 1:, :]  # remove cls token

                out = x.reshape(B, 14, 14, C).permute(0, 3, 1, 2).contiguous()
                reshaped_features.append(out)
        sequence_output = encoder_outputs.last_hidden_state[-1]
        sequence_output = self.layernorm(sequence_output)
        pooled_output = self.pooler(sequence_output) if self.pooler is not None else None

        return BaseModelOutputWithPooling(last_hidden_state=reshaped_features, pooler_output=pooled_output)
