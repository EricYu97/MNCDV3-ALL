...
from transformers.file_utils import ModelOutput, is_scipy_available, requires_backends
...
# Here the weights mismatched messages are suppressed
...
class WS_MaskCDLoss(nn.Module):
    def __init__(self, config: Mask2FormerConfig, weight_dict: dict[str, float], k: int = 5):
        """
        ...
        self.num_labels = 1 # WS-MaskCD only has 1 class: Change vs No Change
...
class MaskCDPixelLevelModule(nn.Module):
    def __init__(self, config: Mask2FormerConfig):
        """
        Pixel Level Module proposed in [Masked-attention Mask Transformer for Universal Image
        Segmentation](https://huggingface.co/papers/2112.01527). It runs the input image through a backbone and a pixel
        ...
        self.cd_decoder = Mask2FormerPixelDecoder(config, feature_channels=self.encoder.channels)

    def forward(self, pixel_values_pre: Tensor, pixel_values_post: Tensor, output_hidden_states: bool = False) -> Mask2FormerPixelLevelModuleOutput:

        backbone_features_pre = self.encoder(pixel_values_pre).feature_maps
        backbone_features_post = self.encoder(pixel_values_post).feature_maps

        backbone_features_cd=tuple([torch.abs(f_pre-f_post) for f_pre, f_post in zip(backbone_features_pre, backbone_features_post)])

        cd_decoder_output = self.cd_decoder(backbone_features_cd, output_hidden_states=output_hidden_states)
        seg_decoder_output=self.decoder(backbone_features_pre, output_hidden_states=output_hidden_states)

        seg_output=Mask2FormerPixelLevelModuleOutput(
            encoder_last_hidden_state=backbone_features_pre[-1],
            encoder_hidden_states=tuple(backbone_features_pre) if output_hidden_states else None,
            decoder_last_hidden_state=seg_decoder_output.mask_features,
            decoder_hidden_states=seg_decoder_output.multi_scale_features,
        )

        cd_output=Mask2FormerPixelLevelModuleOutput(
            encoder_last_hidden_state=backbone_features_cd[-1],
            encoder_hidden_states=tuple(backbone_features_cd) if output_hidden_states else None,
            decoder_last_hidden_state=cd_decoder_output.mask_features,
            decoder_hidden_states=cd_decoder_output.multi_scale_features,
        )
        return {"seg_output": seg_output, "cd_output": cd_output}
...
@auto_docstring
class WS_MaskCDModel(Mask2FormerModel):
    main_input_name = "pixel_values"

    def __init__(self, config: Mask2FormerConfig):
        super().__init__(config)

        self.cd_pixel_level_module = MaskCDPixelLevelModule(config)
        self.cd_transformer_module = Mask2FormerTransformerModule(in_features=config.feature_size, config=config)

        self.post_init()

        # self.cd_transformer_module.load_state_dict(self.transformer_module.state_dict())
    ...
    def forward(
        ...
        **kwargs,
        ...
        pixel_values_t1, pixel_values_t2 = pixel_values[:batch_size//2], pixel_values[batch_size//2:]

        if pixel_mask is None:
            pixel_mask = torch.ones((batch_size, height, width), device=pixel_values.device)

        pixel_level_module_output = self.cd_pixel_level_module(
            pixel_values_pre=pixel_values_t1, pixel_values_post=pixel_values_t2, output_hidden_states=output_hidden_states
        )

        pixel_level_module_output_seg, pixel_level_module_output_cd = pixel_level_module_output["seg_output"], pixel_level_module_output["cd_output"]

        transformer_module_output = self.transformer_module(
            multi_scale_features=pixel_level_module_output_seg.decoder_hidden_states,
            mask_features=pixel_level_module_output_seg.decoder_last_hidden_state,
            output_hidden_states=True,
            output_attentions=output_attentions,
        )

        cd_transformer_module_output=self.cd_transformer_module(
            multi_scale_features=pixel_level_module_output_cd.decoder_hidden_states,
            mask_features=pixel_level_module_output_cd.decoder_last_hidden_state,
            output_hidden_states=True,
            output_attentions=output_attentions,
        )
        ...
        if output_hidden_states:
            encoder_hidden_states = pixel_level_module_output_seg.encoder_hidden_states
            pixel_decoder_hidden_states = pixel_level_module_output_seg.decoder_hidden_states
            transformer_decoder_hidden_states = transformer_module_output.hidden_states
            transformer_decoder_intermediate_states = transformer_module_output.intermediate_hidden_states

        output = Mask2FormerModelOutput(
            encoder_last_hidden_state=pixel_level_module_output_seg.encoder_last_hidden_state,
            pixel_decoder_last_hidden_state=pixel_level_module_output_seg.decoder_last_hidden_state,
            transformer_decoder_last_hidden_state=transformer_module_output.last_hidden_state,
            encoder_hidden_states=encoder_hidden_states,
            pixel_decoder_hidden_states=pixel_decoder_hidden_states,
            transformer_decoder_hidden_states=transformer_decoder_hidden_states,
        ...
        cd_encoder_hidden_states = None
        cd_pixel_decoder_hidden_states = None
        cd_transformer_decoder_hidden_states = None
        cd_transformer_decoder_intermediate_states = None

        if output_hidden_states:
            cd_encoder_hidden_states = pixel_level_module_output_cd.encoder_hidden_states
            cd_pixel_decoder_hidden_states = pixel_level_module_output_cd.decoder_hidden_states
            cd_transformer_decoder_hidden_states = cd_transformer_module_output.hidden_states
            cd_transformer_decoder_intermediate_states = cd_transformer_module_output.intermediate_hidden_states

        cd_output = Mask2FormerModelOutput(
            encoder_last_hidden_state=pixel_level_module_output_cd.encoder_last_hidden_state,
            pixel_decoder_last_hidden_state=pixel_level_module_output_cd.decoder_last_hidden_state,
            transformer_decoder_last_hidden_state=cd_transformer_module_output.last_hidden_state,
            encoder_hidden_states=cd_encoder_hidden_states,
            pixel_decoder_hidden_states=cd_pixel_decoder_hidden_states,
            transformer_decoder_hidden_states=cd_transformer_decoder_hidden_states,
            transformer_decoder_intermediate_states=cd_transformer_decoder_intermediate_states,
            attentions=cd_transformer_module_output.attentions,
            masks_queries_logits=cd_transformer_module_output.masks_queries_logits,
        )



        if not return_dict:
            output = tuple(v for v in output.values() if v is not None) + tuple(v for v in cd_output.values() if v is not None)


        # print(output.keys())
        return {"Seg_T1": output, "CD": cd_output}

@auto_docstring(
    custom_intro="""
    The Mask2Former Model with heads on top for weakly supervised semantic change detection.
    """
)
class WS_MaskCD(Mask2FormerPreTrainedModel):
    main_input_name = "pixel_values"
    ...
    def __init__(self, config: Mask2FormerConfig, topk=5):
        ...
        # self.model.cd_transformer_module.load_state_dict(self.model.transformer_module.state_dict())

    @classmethod
    def from_pretrained(cls, *args, **kwargs):
        model = super().from_pretrained(*args, **kwargs)

        model.model.cd_transformer_module.load_state_dict(
            model.model.transformer_module.state_dict()
        )
        model.model.cd_pixel_level_module.cd_decoder.load_state_dict(
            model.model.cd_pixel_level_module.decoder.state_dict()
        )

        # Here we freeze the decoder part for seg T1
        model.model.transformer_module.requires_grad_=False
        model.model.cd_pixel_level_module.decoder.requires_grad_=False
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        weak_change_labels: Optional[list[Tensor]] = None,
        **kwargs,
        ...
        class_queries_logits_seg = ()
        class_queries_logits_cd = ()
        for decoder_output_seg in outputs_seg.transformer_decoder_intermediate_states:
            class_prediction_seg = self.class_predictor(decoder_output_seg.transpose(0, 1))
            class_queries_logits_seg += (class_prediction_seg,)

        masks_queries_logits_seg = outputs_seg.masks_queries_logits

        auxiliary_logits_seg = self.get_auxiliary_logits(class_queries_logits_seg, masks_queries_logits_seg)

        if mask_labels is not None and class_labels is not None:
            loss_dict_seg = self.get_loss_dict(
                masks_queries_logits=masks_queries_logits_seg[-1],
                class_queries_logits=class_queries_logits_seg[-1],
                mask_labels=mask_labels,
                class_labels=class_labels,
                auxiliary_predictions=auxiliary_logits_seg,
            )
            loss_seg = self.get_loss(loss_dict_seg)

        encoder_hidden_states_seg = None
        pixel_decoder_hidden_states_seg = None
        transformer_decoder_hidden_states_seg = None

        if output_hidden_states:
            encoder_hidden_states_seg = outputs_seg.encoder_hidden_states
            pixel_decoder_hidden_states_seg = outputs_seg.pixel_decoder_hidden_states
            transformer_decoder_hidden_states_seg = outputs_seg.transformer_decoder_hidden_states
        ...
        output_seg = Mask2FormerForUniversalSegmentationOutput(
            loss=loss_seg,
            class_queries_logits=class_queries_logits_seg[-1],
            masks_queries_logits=masks_queries_logits_seg[-1],
            auxiliary_logits=auxiliary_logits,
            encoder_last_hidden_state=outputs_seg.encoder_last_hidden_state,
            pixel_decoder_last_hidden_state=outputs_seg.pixel_decoder_last_hidden_state,
            transformer_decoder_last_hidden_state=outputs_seg.transformer_decoder_last_hidden_state,
            encoder_hidden_states=encoder_hidden_states_seg,
            pixel_decoder_hidden_states=pixel_decoder_hidden_states_seg,
            transformer_decoder_hidden_states=transformer_decoder_hidden_states_seg,
            attentions=outputs_seg.attentions,
        )
        ...
        auxiliary_logits_cd = self.get_auxiliary_logits(class_queries_logits_cd, masks_queries_logits_cd)

        
        if weak_change_labels is not None:
            loss_dict_cd = self.criterion_ws_cd(
        ...
        # for k,v in loss_dict_seg.items():
        #     if "cross_entropy" in k:
        #         print(k,v)
        
        encoder_hidden_states_cd = None
        pixel_decoder_hidden_states_cd = None
        transformer_decoder_hidden_states_cd = None

        if output_hidden_states:
            encoder_hidden_states_cd = outputs_cd.encoder_hidden_states
            pixel_decoder_hidden_states_cd = outputs_cd.pixel_decoder_hidden_states
            transformer_decoder_hidden_states_cd = outputs_cd.transformer_decoder_hidden_states


        output_cd = Mask2FormerForUniversalSegmentationOutput(
            loss=loss_cd,
            class_queries_logits=class_queries_logits_cd[-1],
            masks_queries_logits=masks_queries_logits_cd[-1],
            auxiliary_logits=auxiliary_logits,
            encoder_last_hidden_state=outputs_cd.encoder_last_hidden_state,
            pixel_decoder_last_hidden_state=outputs_cd.pixel_decoder_last_hidden_state,
            transformer_decoder_last_hidden_state=outputs_cd.transformer_decoder_last_hidden_state,
            encoder_hidden_states=encoder_hidden_states_cd,
            pixel_decoder_hidden_states=pixel_decoder_hidden_states_cd,
            transformer_decoder_hidden_states=transformer_decoder_hidden_states_cd,
            attentions=outputs_cd.attentions,
        )

        return {"outputs_seg":output_seg, "outputs_cd":output_cd}
...
if __name__ == "__main__":
    model=WS_MaskCD.from_pretrained("facebook/mask2former-swin-base-ade-semantic", ignore_mismatched_sizes=True).cuda()

    # model.model.cd_transformer_module.load_state_dict(model.model.transformer_module.state_dict())
    # model.model.cd_pixel_level_module.cd_decoder.load_state_dict(model.model.cd_pixel_level_module.decoder.state_dict())

    for (n1, p1), (n2, p2) in zip(
        model.model.cd_pixel_level_module.cd_decoder.named_parameters(),
        model.model.cd_pixel_level_module.decoder.named_parameters()
    ):
        # diff = (p1 - p2).abs().max()
        # print(n1, diff.item())
    ...
    # import torch
    # pixel_values=torch.randn(8,3,224,224).cuda()

    # outputs=model(pixel_values=pixel_values)
    # print(outputs["outputs_seg"].keys(), outputs["outputs_cd"].keys())
    # print(outputs["outputs_seg"].class_queries_logits.shape, outputs["outputs_seg"].masks_queries_logits.shape)
    # print(outputs["outputs_cd"].class_queries_logits.shape, outputs["outputs_cd"].masks_queries_logits.shape)

    # print(outputs["outputs_cd"].class_queries_logits-outputs["outputs_seg"].class_queries_logits)